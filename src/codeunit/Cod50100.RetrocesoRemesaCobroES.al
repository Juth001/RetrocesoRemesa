codeunit 50100 "Retroceso Remesa Cobro ES"
{
    ...
    var
        TextoConfirmacion: Label 'Se va a retroceder la remesa de cobro %1\Banco: %2  |  Importe: %3  |  Fecha: %4\\\· Se invertirá el asiento contable del registro.\· Los movimientos de cliente serán desaplicados.\· Los documentos volverán a Cartera listos para ser remesados.\¿Desea continuar?', Comment = '%1=No., %2=Banco, %3=Importe, %4=Fecha';
        TextoExito: Label 'La remesa %1 ha sido revertida correctamente.\Los documentos han vuelto a Cartera listos para ser remesados de nuevo.', Comment = '%1=No.';
        NumTransaccion: Integer;
    begin
        ValidarConfiguracionCartera();
        ValidarDocumentosRemesa(RemesaRegistrada."No.", "Cartera Document Type"::Receivable);
        NumTransaccion := ObtenerNumTransaccion(RemesaRegistrada."No.", RemesaRegistrada."Bank Account No.");

        if not Confirm(TextoConfirmacion, false,
            RemesaRegistrada."No.",
            RemesaRegistrada."Bank Account No.",
            RemesaRegistrada.Amount,
            RemesaRegistrada."Posting Date")
        then
            Error('');

        DesaplicarClientesPorTransaccion(NumTransaccion);
        PostearDiarioInverso(RemesaRegistrada."No.", NumTransaccion);
        ProcesarDocumentosCartera(RemesaRegistrada);
    ...
    var
        ...
        ListaEfectos: Text;
        ErrEfectos: Label 'No se puede retroceder la remesa %1 porque contiene efectos (no facturas):\%2\Los efectos deben gestionarse mediante la acción "Rechazar".', Comment = '%1=No., %2=Lista';
        ErrImpresa: Label 'No se puede retroceder la remesa %1 porque ha sido impresa (%2 veces).\Anule el efecto impreso antes de retroceder.', Comment = '%1=No., %2=Num.';
    begin
        if RemesaReg.Get(NumRemesa) then
            if RemesaReg."No. Printed" > 0 then
                Error(ErrImpresa, NumRemesa, RemesaReg."No. Printed");
    ...
    var
        ...
        ErrNoEncontrado: Label 'No se encontraron apuntes contables activos para la remesa %1. Es posible que ya haya sido revertida.', Comment = '%1=No.';
    begin
    ...
    var
        ...
        DesaplicarApunte: Codeunit "CustEntry-Apply Posted Entries";
    begin
        ApunteCliente.SetCurrentKey("Transaction No.");
        ApunteCliente.SetRange("Transaction No.", NumTransaccion);
        if not ApunteCliente.FindSet() then
            exit;
        repeat
            if not ApunteCliente.Open then
                DesaplicarApunte.UnApplyCustLedgEntry(ApunteCliente."Entry No.");
    ...
    begin
        ...
        repeat
            ...
            LineaDiario.Description := CopyStr('Retroceso remesa ' + NumRemesa, 1, MaxStrLen(LineaDiario.Description));
    ...
    begin
        DocCarteraPost.SetRange("Bill Gr./Pmt. Order No.", RemesaRegistrada."No.");
        DocCarteraPost.SetRange(Type, "Cartera Document Type"::Receivable);
        if not DocCarteraPost.FindSet() then
            exit;
        repeat
            // -- 1. Insertar en Cartera Doc. con copia directa de campos --
            // NO se usa Validate para evitar que OnValidate busque otro CLE
            // y sobreescriba Posting Date, Amount, Dimension Set ID, etc.
            // Entry No. = el mismo de Posted Cartera Doc. = CLE Entry No. original
            DocCartera.Init();
            ...
            // Si divisa vacía → DL = importe en divisa
            // Si divisa extranjera → convertir con tipo de cambio de la fecha de registro
            CalcularImportesDL(DocCartera);

            // -- Actualizar situación del movimiento de cliente → "Cartera" --
            // Al registrar la remesa el sistema puso "Remesa registrada".
            // Al retroceder debe volver a "Cartera" para que el documento
            // esté disponible para ser remesado de nuevo.
            ActualizarSituacionCLE(DocCarteraPost."Entry No.");
        ...
        // -- Eliminar cabecera de Posted Bill Group --
    ...
    begin
        if not CustLedgEntry.Get(EntryNo) then
            exit;
